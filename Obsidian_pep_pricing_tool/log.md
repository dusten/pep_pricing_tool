...
## [2026-07-15] feature | Activity dashboard: independent toggles, generalized click tracking, deploy incident

- Follow-up to backlog #67's first bar-chart cut: user asked for each card's Day/Week/Month toggle to work independently (previously all 4 shared one ref, so clicking any one switched all four), asked to remove the older single-window Activity stat-grid (superseded by the 13-period bar cards), and asked whether WhatsApp click tracking was complete -- it wasn't covering the Website link, so extended it, then mid-work the user also asked to track clicks on the CAS/PubChem link (Comparison page) and to add a Downloads/Exports card.
- `trendGranularity` changed from a single ref to a `reactive()` object keyed per metric, so each of the 7 cards (Signups/Logins/Searches/WhatsApp/Website/CAS/Downloads) now toggles independently. Deleted the old activity_stats.php endpoint and its stat-grid UI entirely.
- Generalized `pc_whatsapp_clicks` (migration 034) into `pc_outbound_link_clicks` (migration 035): VARCHAR `link_type` column instead of a single-purpose table, `vendor_id` made nullable, new nullable `product_id` (CAS clicks are product-scoped, not vendor-scoped). One endpoint (`POST /track/link-click`) replaces the old whatsapp-only one. Downloads/Exports reuses the existing `pc_user_audit_log` export actions (`export_full`, `export_comparison_xlsx`, `export_personal_data`, `export_comparison_csv`) -- no new tracking needed there.
- **Deploy incident**: `deploy.sh --all` applies schema.sql before migrations. Since schema.sql was updated to define `pc_outbound_link_clicks` directly (the fresh-install target shape), that pass created an empty stub table under the new name *before* migration 035 ran its `ALTER ... RENAME TABLE pc_whatsapp_clicks TO pc_outbound_link_clicks` -- the rename collided with the just-created empty stub and the deploy aborted mid-migration (`set -euo pipefail`). Diagnosed via direct read-only queries on the live DB (confirmed `pc_whatsapp_clicks` still had its 2 real rows and had already received the ALTER's new columns; `pc_outbound_link_clicks` existed but was empty; migration 035 was not yet recorded in `pc_migrations`). With explicit user sign-off, manually completed the migration by hand on production (dropped the empty stub, renamed the real table over, recorded 035 as applied) -- both original click rows survived intact. Rewrote migration 035 to guard on `information_schema.tables` (only alter+rename if the old table still exists and the new one doesn't; drop the old one as a no-op leftover otherwise) so the same schema.sql-vs-migration collision can't recur, including on a fresh install. Re-ran `deploy.sh --all` clean end to end; `php -l` clean on both touched backend files.

## [2026-07-15] fix | Bell-curve gate switched from 75% vendor coverage to absolute floor of 8

- User noticed not all items show a bell-curve chart and asked how it's generated (confirmed fully dynamic -- recomputed live on every open from the same `runComparisonQuery()` path the Comparison table itself uses, no caching/pre-generation) -- then flagged that Adamax 5mg (12 vendors) and 10mg (11 vendors) felt well-sampled enough to qualify but didn't.
...
## [2026-07-15] query | Vendor-suggestions feature spec drafted (backlog #69)

- User wants to let both vendor reps and regular customers suggest a new vendor: contact details + a pricing file, virus-scanned and parsed (a strict downloadable CSV template parses instantly; anything else — PDF/XLSX/image/ZIP — goes through the existing async Claude extraction queue). The submitter always gets a private price-score report showing how the vendor's prices stack up against the live market; a separate manual admin-accept decision is what actually creates a real catalog vendor and commits its prices through the existing import machinery.
- Key decisions confirmed with the user before drafting: both submitter types via a `relationship` field (vendor_rep/customer/other); outcome is always score + a separate admin accept, never auto-accept; parsing is template-first with a pipeline fallback, not a single one-size path. Reuses nearly the entire existing upload/scan/extraction/review-queue stack (`vendors/files.php`, `malware_scan.php`, `vendor_file_processor.php`, the COA/pending-imports review-queue pattern, vendor helpers, `pricePerUnit()`/`findExactProductMatch()`, Brevo templates, rate limiting, admin audit logging) rather than building new plumbing.
- Gated to `pc_users.test_account = 1` users throughout the build (server-side 404 via `requireSuggestionAccess()`, not just nav hiding) — the flag already exists in the DB but isn't yet exposed via `/api/me`, which is part of this work. Not built yet; filed as backlog #69 and spec'd in full at wiki/analyses/2026-07-15-vendor-suggestions-spec.md (DDL, status lifecycle, backend/frontend changes, a naive percentile-based vendor score, 3-phase build plan, verification checklist).

## [2026-07-15] feature | Vendor suggestions Phase 1 built (backlog #69)

- Built everything in the spec's Phase 1 list, same day as the spec: migration 036 (`pc_vendor_suggestions`, mirrored in `schema.sql`); `userShape()` now exposes `test_account`; `backend/lib/vendor_suggestions.php` (strict-template CSV parser, `scoreSuggestionPrices()` percentile scorer, `requireSuggestionAccess()` build-phase gate); `backend/api/vendor_suggestions/index.php` (submit + list, reusing the file-gate/malware-scan/rate-limit shape from `vendors/files.php`, plus a durable 3-per-7-days DB count on top of the hourly memcached limit, and soft dedup flagging against both the catalog and other pending suggestions); `backend/api/admin/vendor_suggestions.php` (list + accept/reject — accept creates the real `pc_vendors` row, moves the file into `storage/vendor_files/`, and reuses `commitExtractionResult()` unmodified for the price commit); three Brevo email senders (scored/accepted/rejected, built inline rather than as new `.html` template files); routes in `public/index.php`; frontend `auth.isTestAccount` computed + router guard + `BottomNav.vue` link, new `SuggestVendorView.vue`, and admin `VendorSuggestionsTab.vue`.
...
## [2026-07-15] feature | Vendor suggestions Phase 2 built (backlog #69)

- Built the spec's Phase 2 list: `buildExtractionUserContent(string $fullPath, string $fileType, ?string &$sheetNote): array` pulled out of `processVendorFile()` in `backend/lib/vendor_file_processor.php` (behavior identical, `processVendorFile()` now just calls it); `processSuggestion(array $s, string $model): void` added to `backend/lib/vendor_suggestions.php` (build content → `callClaudeExtraction(..., null)` → score → `scored` + scored email, or `parse_failed` on any failure — no email on `parse_failed`, matching the existing inline template-CSV path in `vendor_suggestions/index.php`); a second loop in `backend/cron/process_async_queue.php` inside the same `GET_LOCK('pc_process_async_queue')`, atomically claiming `pending_parse` rows (`UPDATE ... WHERE status='pending_parse'` + `rowCount()` check, since — unlike vendor files — nothing pre-claims these before the cron sees them) and logging to `pc_maintenance_runs` as job `process_vendor_suggestions`.
- Deployed via `deploy.sh` (code-only, no schema change); smoke check passed. `php -l` clean on all 4 touched/new backend files. Regression check on the shared refactor: `diagnostic_scripts/check_build_extraction_content.php` re-runs `buildExtractionUserContent()` against 3 real already-processed vendor files (pdf/xlsx/image) with no Claude call and no writes, asserting the exact content-block shape (document/text/image blocks, media types) the old inline code produced — all passed.
- End-to-end pipeline test (real Claude call): inserted a `pending_parse` test suggestion row pointing at a copy of a real 18KB vendor xlsx price list, ran `backend/cron/process_async_queue.php` manually on the server. Row transitioned `pending_parse` → `processing` → `scored` with real `extracted_json` (244 rows) and `score_json` (`matched_rows: 116, vendor_score: 73`); `pc_claude_call_log` id 70 shows `vendor_file_id NULL, model claude-sonnet-5, http_status 200, parsed_ok 1, input_tokens 2665, output_tokens 27265`. Row then marked `rejected` with `admin_note = 'phase 2 pipeline test'` per plan. Test script archived to `diagnostic_scripts/`.
- **Found, not fixed (deviation, flagged for follow-up):** per-suggestion storage subdirectories created at runtime by PHP-FPM (`mkdir($dir, 0770, true)` in `vendor_suggestions/index.php`) land as `apache:apache 750/644` on disk, not `ec2-user:apache`. `ec2-user` (the identity `process_async_queue.php` runs as via cron) is **not** a member of the `apache` group on this server, so it cannot traverse into any apache-created suggestion subdirectory — confirmed directly (`ls` as ec2-user → Permission denied on a real prior submission's directory). This silently breaks the Phase 2 cron for every real non-template suggestion file until fixed; the pipeline test above only succeeded because the test file was deliberately re-chowned to `ec2-user:apache` per the verification plan's own instructions. Same latent gap likely affects the Phase 1 async-vendor-files cron path for oversized PDFs (never yet triggered in production, so never surfaced). Root cause: no `usermod -aG apache ec2-user` step in `setup-al2023.sh`/`add-price-site.sh`, and no setgid bit on the storage dirs. Out of this task's file-scope (infra/deploy-script change, not a Phase 2 code change) — needs a deliberate fix before Phase 2 works on any real submission.

## [2026-07-15] fix | ec2-user added to apache group (Phase 2 cron blocker)

Follow-up to the deviation flagged in the Phase 2 build: PHP-FPM creates per-suggestion upload
dirs/files as `apache:apache` (750/644), and the async cron runs as `ec2-user`, which was not in
the `apache` group — so the Phase 2 worker could not read any real submission's file (confirmed
`Permission denied` on a real prior upload; the same gap was dormant in Phase 1's async
vendor-files PDF path, never triggered because no production PDF has crossed the 2MB threshold).
Fixed live (`sudo usermod -aG apache ec2-user`, verified a fresh ec2-user session lists and reads
apache-created suggestion files) and permanently in `setup.sh` + `add-price-site.sh` next to the
storage-dir provisioning. Phase 2 is now actually operable for real traffic.
...
## [2026-07-22] fix | Vendor ranking payment bonus changed from flat PayPal bonus to 7-tier buyer-protection ranking

`backend/api/vendors/ranked.php` — replaced the flat `+10 if paypal/pyusd` bonus with a 7-tier lookup (`paypal`10, `zelle`8, `alibaba`6, `wise`5, `alipay`4, `credit_card`3, everything else incl. `pyusd`1, no rows on file 0), best-accepted-method-wins (not additive). `VendorRankingModal.vue` breakdown line now shows the tier label + bonus instead of PayPal yes/no. Verified live against `/api/vendors/ranked` (DB-inserted session token, no browser involved, deleted after): vendor 1 (Zhongke Meiye — `paypal`+crypto) → bonus 10/"PayPal", proving best-method-wins over its own crypto methods. Vendor 10 (Premipeptides — `wise`,`alibaba`,`btc`,`western_union`) → bonus 6/"Alibaba" (beats wise's 5). Vendor 26 (PeptideOS — `btc` only) → bonus 1/"Other/Crypto". Vendor 23 (Tiancheng Biotechnology — no payment rows) → bonus 0/no label. All four scores hand-computed and matched the live response exactly. No live vendor has `pyusd` on file, so the "pyusd ≠ paypal" rule was verified via a standalone PHP assertion harness using the same tier table instead. `php -l` clean, `bash deploy.sh` + smoke check passed. Backlog #70 updated in place (refinement, not a new item).

## [2026-07-22] feature | PreCompact hook auto-checkpoints wiki/log/session notes before compaction

User asked for something in CLAUDE.md that automatically writes log.md/wiki/session notes before context gets compacted (~95% used) or before /compact, with no user intervention — and to commit this to memory. A CLAUDE.md text instruction alone can't guarantee automatic, no-intervention behavior (the harness executes hooks, not prose preferences), so used the update-config skill to add a real `PreCompact` hook in `.claude/settings.json` instead — two entries (`matcher: "auto"` and `matcher: "manual"`, covering both trigger types) running a `jq`-built command that injects a directive via `hookSpecificOutput.additionalContext`: append any unlogged work to `log.md`, create/append today's `sessions/YYYY-MM-DD.md`, add missing `index.md` rows, then commit and push — before continuing. Validated via `jq -e` schema checks on both entries (exit 0). Documented plainly in CLAUDE.md as a "best-effort, not a hard guarantee" mechanism: the hook deterministically fires and injects the reminder every time, but actually authoring the wiki content still requires a real model turn, so a compaction landing mid-tool-call could still skip the checkpoint. `.claude/settings.json` had never been committed to this repo before (confirmed not gitignored, just sitting untracked all session) — committed it for the first time alongside this change so the hook is a durable, shared project setting. Saved as project memory `project_precompact_wiki_hook.md` per the explicit "commit this to memory" instruction.

## [2026-07-22] fix | Vendor ranking "no PayPal" report was a stale browser tab, not a real bug; added vendor-card click-through

User reported the vendor-ranking pop-up "no longer show anyone taking paypal." Investigated methodically: (1) confirmed via direct SQL that 14 real active/visible vendors have `paypal` on file; (2) read `ranked.php`'s tier logic, found nothing wrong; (3) replicated the tier algorithm standalone against live data for vendor 1 — correctly computed bonus 10/"PayPal"; (4) inspected the raw memcached `comparison_data:vendor_ranking` cache entry directly — already on the current schema (`payment_tier_label` present), 14 vendors correctly labeled PayPal including vendor 1; (5) confirmed the deployed JS bundle (`DashboardView-*.js`, built today) contains `payment_tier_label` with zero references to the old `has_paypal` field anywhere in `dist/assets`. Backend, cache, and deployed frontend were all already correct — root cause was almost certainly a browser tab left open from before today's earlier tier-refactor deploy, still running old in-memory JS that read a field the API no longer returns (SPA doesn't refetch its own bundle without a hard reload). No server-side fix needed; live-verified in a fresh browser session that PayPal renders correctly for multiple vendors (Lucy, keruihk.net, Purelypep Factory, etc., all +10/"PayPal").

Also implemented the second half of the request: `VendorRankingModal.vue` rows are now clickable, opening the existing `VendorCard.vue` contact popup for that vendor (`openVendorId` ref, `<VendorCard v-if="openVendorId" :vendor-id="openVendorId" @close="openVendorId = null" />` — same pattern as `ComparisonView.vue`). Each row's body became a `<button>` (`.vrm-body-btn`) instead of a plain div, underline-on-hover to signal it's clickable. `bash deploy.sh` + smoke check passed. Live-verified via a DB-inserted throwaway test session (test_account=1, id 35): clicked Lucy's row inside the ranking modal, `VendorCard` opened correctly showing her contact info (WhatsApp/phone/email/Discord/Telegram), no console errors, layered cleanly on top of the ranking modal. Test user + session deleted afterward, localStorage cleared, browser tab closed.

## [2026-07-27] scan | Weekly clipping scan — no new files

- Scanned `raw/clippings/`: 5 files found, all previously ingested on 2026-06-30
- No new source pages, entity pages, or variant sightings this week

## [2026-07-23] feature | Design review + modernization pass (typeface, woodblock motif, Cart color fix, Pricing hero)

User invoked the frontend-design skill for a review + "modernize the design a bit." Reviewed Dashboard/Comparison/Pricing/Cart against the existing navy/gold system in `main.css` — palette and component system already solid, but typography was 100% system UI fonts with zero connection to the "Japanese-woodblock aesthetic" the design system's own header comment claims; Pricing was a generic SaaS pricing-page template; Cart's cheapest-vendor list painted every full-coverage row the same green regardless of actual price, making color decorative instead of informational. User greenlit typeface+motif, the Cart fix, and a Pricing hero redo. Added Fraunces as `--font-display` (self-hosted single variable woff2, since CSP is `default-src 'self'` and would've needed changing for an external font CDN) for hero moments only (auth title, Dashboard welcome, Pricing h1) — never body text. Added `.eyebrow-woodblock`, a hairline double-rule motif flanking eyebrow labels (echoing a ukiyo-e title cartouche), chosen over a tiled wave-pattern background for lower risk of looking wrong without visual iteration. Cart: green highlight + new "Best price" badge now target only the actual cheapest vendor (`cart.cheapestFullCoverage`), not every full-coverage row. Pricing hero: new headline copy, motif eyebrow, 44px display serif. `bash deploy.sh` passed; live-verified via the user's own already-logged-in session (read-only, undisturbed) — serif + motif render correctly, Cart shows exactly one highlighted row, zero console errors. See `sessions/2026-07-23.md` for full detail.

Same-day follow-up: user asked for the same treatment on Comparison. That page has no in-body hero (its `AppLayout title` prop only feeds the small shared TopBar label, which stays untouched — a full Pricing-style hero would eat vertical space on the app's most data-dense page). Added a compact one-line header above the filter bar instead: `.eyebrow-woodblock` + a 21px display-serif line ("Every vendor, side by side." — deliberately echoing Pricing's "Every vendor. One true price." as a small recurring voice thread between the two pages). `bash deploy.sh` passed; live-verified — renders correctly, filter bar and data start immediately below with no meaningful space lost, zero console errors.

## [2026-07-27] fix | Emoji "obj" box: fixed a real Fraunces unicode-range bug, diagnosed the actual report to WhatsApp's clipboard behavior

User reported pasted emoji showing broken box glyphs in the admin vendor shipping-note field. Investigation found and fixed a real latent bug unrelated to the report: the Fraunces `@font-face` added 2026-07-23 had no `unicode-range`, so per spec it claimed all of Unicode by default — any character it lacks a glyph for (emoji, non-Latin text) would tofu instead of falling back to Georgia/serif anywhere `--font-display` is used (e.g. a display name with an emoji in the Dashboard welcome heading). Fixed with an explicit Latin-only `unicode-range`; also added emoji-font fallbacks to `--font-sans`. That fix doesn't explain the actual report though — the shipping textarea doesn't use `--font-display` at all, and typing the exact reported emoji live into that exact field rendered correctly. Asked the user where the text came from: WhatsApp — which renders emoji as image sprites and can leave U+FFFC (object replacement character, the literal source of an "obj" box) in the clipboard's plain-text fallback instead of the real codepoint; unrecoverable client-side, not a CSS/font bug. Shipped a small defensive fix anyway since vendor onboarding regularly involves pasting vendor-supplied text: `VendorsTab.vue`'s `save()` now strips U+FFFC from `shipping_note`/`notes` before the API call, so the artifact never gets saved as visible garbage. Verified the regex logic directly rather than creating a throwaway vendor in the real DB. See `sessions/2026-07-27.md`.

## [2026-07-27] feature | Tablet-form product tracking (is_tablet flag, mirrors is_raw_material)

User kept running into vendors selling tablet/oral forms and asked how to track them, floating "maybe a different table." Investigated the existing `pc_products`/`pc_specifications`/`pc_prices` schema first — `is_raw_material` already solves the identical problem ("one product, more than one form, needs its own filter+badge in the comparison grid, no changes to the $/unit math") so no new table was needed; a new `is_tablet` boolean on `pc_specifications` mirroring `is_raw_material` end-to-end was the right scope. Confirmed via AskUserQuestion: mix of same-compound-different-form and tablet-only products, full price-comparison integration (not just lightweight tagging), pricing convention still unknown/varies. Delegated the build to a Sonnet 5 subagent with a fully-specified prompt (migration 041, `findOrCreateSpec()`, the extraction prompt, comparison filters, Comparison page checkbox+badge, Review Queue checkbox) after pre-identifying a subtle correctness requirement myself: the unique key has to widen to `(product_id, spec_label, is_tablet)` and BOTH spec-lookup queries (`findOrCreateSpec()` and `pending_imports.php`'s separate auto-approve-matching-vendor `$specCheck` query) must filter on `is_tablet`, not just `spec_label` — unlike raw-material's "1g" label, which never collides with a finished dose, a tablet dose plausibly shares the exact same label as an injectable dose of the same product (both "5mg"). Independently verified the agent's report before pushing: read the actual diff for both correctness-critical lookups (both fixed correctly), confirmed no other call sites of `findOrCreateSpec()` were missed, confirmed `commitExtractionResult()` (used by both the vendor-file and vendor-suggestion accept flows) doesn't bypass the fix since it only ever writes through `pc_pending_imports` → the same approve endpoint, re-ran `SHOW CREATE TABLE pc_specifications` myself to confirm the live schema, and confirmed all the agent's throwaway test data (product/vendor/specs/prices) was actually gone. Pushed as `2b352bd`.

## [2026-07-27] fix | Review Queue: real product picker, auto-commit low-risk new_spec rows, CJC-1295 DAC watchlist + alias backfill

Same session, user raised two connected pain points: 111 items sitting in the admin Review Queue, and a specific recurring mismatch — vendor listings for "CJC-1295 with DAC" kept getting suggested against the existing-but-wrong "CJC-1295 without DAC" product (a genuinely different compound, not a spelling variant — DAC is a half-life-changing modification), forcing the user to just skip the row since the UI offered no way to map onto a *different* specific existing product. Root-caused via real data: `pc_pending_imports` breakdown was `new_product` 30 / `new_spec` 31 / `name_mismatch` 46 / `incomplete_spec` 4; the CJC mismatch traced to "CJC-1295 without DAC" having accumulated 11 alias spellings over time vs. "with DAC" having only 1 — the fuzzy candidate-suggestion logic is pure Levenshtein distance across every product+alias, so 11x more spelling variants to compare against skewed it toward the wrong product almost regardless of which one was semantically correct. Neither was on the existing variant-compound watchlist (which already exists for this exact class of problem — Epithalon/TB-500 — just never got CJC-1295 added).

Fixed directly myself (data-content decision, kept tight control rather than delegating): backfilled 5 more alias spellings for "CJC-1295 with DAC" (product 59) via direct SQL, closing the imbalance to 7-vs-11.

Delegated the code changes to a Sonnet 5 subagent: (1) added CJC-1295 with/without-DAC to `VARIANT_COMPOUND_WATCH_NAMES` in `claude.php`; (2) replaced the Review Queue's "map onto the one suggested candidate" checkbox (defaulted checked — the actual bug that made "just skip it" the only safe option) with a real search+select over the full ~286-product catalog, defaulting safely to create-new, with the suggested candidate now only an explicit opt-in button; backend gained a `create_new` flag since `product_id` being absent previously still silently fell back to the stored candidate; (3) `vendor_file_processor.php`: once `findExactProductMatch()` succeeds, a spec that doesn't exist yet on that product is no longer parked as `new_spec` for review — product identity isn't in question there (only new_product/name_mismatch carry real identity risk), so it auto-commits straight through `findOrCreateSpec()`/`commitPriceRow()`; (4) a one-off backfill script (`migration_scripts/2026-07-27-backfill_new_spec_auto_commit.php`) cleared the 31 pre-existing `new_spec` rows the same way.

Independently verified before considering this done: reviewed the actual diff for all three PHP changes (the `$forceNewProduct` logic, the `findOrCreateSpec()` auto-commit, the watchlist array) and the Vue picker's template/script; re-ran the pending-imports breakdown myself — `new_spec` gone, `new_product`/`name_mismatch`/`incomplete_spec` counts unchanged (30/46/4) as expected; confirmed the CJC watchlist text is live by grepping the deployed `claude.php` directly; confirmed the agent's own throwaway test product/sessions were actually cleaned up. `bash deploy.sh` smoke-checked clean.

## [2026-07-27] fix | Review Queue product picker dropdown was capping results at 40, hiding most of the catalog

User reported the new product picker's dropdown "doesn't show all the products." Root cause: the build spec I wrote for the picker (same session) had it cap `filteredProducts` to 40 results even with no search text entered — with ~286 real products, opening the dropdown blank only ever showed the first 40 alphabetically, silently hiding the rest unless a search term narrowed the match count below 40. Removed the cap entirely — a native `<select>` handles hundreds of options fine, there was no real performance reason for it. `bash deploy.sh` smoke-checked clean.

## [2026-07-30] feature | Comparison page: vendor checkbox list replaced with a payment-method filter + vendor name search

User asked to remove the Comparison page's full vendor checkbox list (every vendor as its own checkbox, `.vendor-checks`) and replace it with a filter by accepted payment method, while keeping the ability to type a vendor's name if that's specifically what's being searched for. Two real design decisions clarified via AskUserQuestion before building: (1) payment-method granularity — grouped into the same 7 buckets already established on the Dashboard's vendor-ranking pop-up (PayPal/Zelle/Alibaba/Wise/Alipay/Credit Card + one combined "Other/Crypto" bucket for the other 14 raw enum values), rather than exposing all 20 raw `pc_vendor_payment_methods.method` values as separate pills; (2) multi-select semantics — ANY/OR (checking PayPal+Zelle shows vendors accepting either), matching how the existing classification-tag filter on this same page already works.

Backend: `comparison_query.php` gained a `payment_methods[]` filter (`parseComparisonFiltersFromGet()`'s 10th element, a new `PAYMENT_FILTER_NAMED_METHODS` const, and an `EXISTS` subquery against `pc_vendor_payment_methods` — named buckets match `method IN (...)`, the "other" bucket matches `method NOT IN` the 6 named ones) — wired through every existing caller (`comparison/index.php`'s cache-hash/query-log compact() calls, both export endpoints, `admin/query_log_rerun.php`). Frontend: the `.vendor-checks` block replaced with a `.vendor-filter-row` — payment pills styled identically to the existing category-tag pills (`.cat-tab`), plus a "Filter by vendor name…" text input. Typing a name computes matching vendor ids client-side against the already-loaded `comparison.vendors` list and feeds the existing `selectedVendors` mechanism (unchanged internally — deep-linked vendor ids from the admin query-log/Calendar pages still work via `initFromQuery()` until the admin actually types something in the new box).

Verified independently before considering this done: real SQL confirmed vendor 1 accepts `paypal`/`zelle`/`alibaba` but not `wise`; curled the live `/api/comparison` endpoint directly (throwaway test session) with 5 filter combinations against vendor 1 and got the exact expected include/exclude result for each; ran an unfiltered vs. `payment_methods[]=paypal` query across the whole catalog and confirmed the distinct-vendor count dropped from 40 to exactly 15 — matching the real `SELECT COUNT(DISTINCT vendor_id) FROM pc_vendor_payment_methods WHERE method='paypal'` count precisely. Live-checked the actual UI: PayPal pill correctly narrows the list view down to vendors like "Lucy" (a vendor already confirmed PayPal-accepting from the 2026-07-22 ranking work); vendor-name search for "Lucy" independently narrows to the same vendor. Zero console errors. `bash deploy.sh` smoke-checked clean, `php -l` clean on all 5 touched backend files.

## [2026-07-30] feature | VendorCard: "Catalog" link to that vendor's filtered Comparison view

Same-day follow-up: user asked for a link on the vendor contact card (`VendorCard.vue`, used from Cart/Dashboard-ranking-modal/Comparison itself) that jumps to the Comparison page pre-filtered to that one vendor. Added a `RouterLink` row ("Catalog → View full price list") reusing `/comparison?vendors={id}` — the same deep-link query param `initFromQuery()` already reads (previously only reachable via the admin query-log replay tool or a Calendar teaser link, never a user-facing entry point).

Two correctness gaps addressed proactively rather than shipping a naive link: (1) landing on Comparison via this link left the new vendor-name search box blank while filtering was silently already active — fixed by having `initFromQuery()` look up the vendor's real `display_name` from the already-loaded `comparison.vendors` list and populate the search box with it for single-vendor deep links (a `skipVendorSearchWatch` guard flag prevents the box's own watcher from re-deriving the id set via substring match right after, which could in principle over-match if another vendor's name happened to be a substring); (2) Vue Router doesn't remount a component for a same-route navigation (only `route.query` changes) — clicking a different vendor's catalog link while already sitting on `/comparison` would otherwise silently do nothing, since `initFromQuery()`/`runSearch()` previously only ran from `onMounted`. Added `watch(() => route.query, () => { initFromQuery(); runSearch() })` to close that gap.

Verified live end-to-end on the real site (own account, no test data needed — read-only navigation): opened Cart's vendor list, clicked "Anna Yiwu" → VendorCard → "View full price list" → landed on `/comparison?vendors=34` with the search box correctly pre-filled "Anna Yiwu" and results narrowed to just her products (cross-route case). Then, while still on that Comparison page, opened a different vendor's card (AMY) from the table header and clicked her catalog link too — URL changed to `?vendors=39` and the table correctly re-filtered to AMY-only with the search box updating to "AMY" and the modal auto-closing (same-route case, the gap fix). Zero console errors both times. `bash deploy.sh` smoke-checked clean.

## [2026-08-01] feature | Price Distribution modal: click vendor → VendorCard; VendorCard now shows accepted payment methods

Two related requests in one pass. (1) `DistributionModal.vue`'s vendor table listed plain vendor names — made them clickable (`.vendor-name-btn`, same pattern as Cart/Comparison/VendorRankingModal) opening the existing `VendorCard.vue` contact popup. Checked the z-index stacking before assuming it'd render correctly: `DistributionModal`'s own backdrop (`main.css`'s shared `.view-backdrop`) is z-index 1000, `VendorCard`'s own backdrop is z-index 100 — on paper this looked like it could render the vendor card underneath, but since `VendorCard` is a DOM child nested inside `.view-backdrop`'s own stacking context, its z-index is only compared against its sibling `.view-card` (z-index auto), so it correctly layers on top — confirmed this empirically live rather than trusting the paper reasoning alone, since CSS stacking-context nesting is easy to get wrong by inspection.

(2) User asked for the vendor card to show accepted payment methods — the backend already returns this (`GET /vendors/{id}/contact` merges in `loadVendorPhonesAndPaymentMethods()`'s `payment_methods` array, added for an earlier feature but never rendered anywhere). Added a "Payment methods" section to `VendorCard.vue` (pill list using the existing `.badge` class, "None on file" fallback for vendors with zero methods) that reuses real human-readable labels. Extracted the 19-entry value/label map that already existed once, inline, in the admin `VendorsTab.vue` form into a new shared `frontend/src/utils/paymentMethods.js` (`PAYMENT_METHODS` array + `paymentMethodLabel()` lookup) rather than copy-pasting a second full copy — `VendorsTab.vue` now imports from it too, so the two can't drift.

Verified live: expanded BPC-157 5mg's distribution chart (42 vendors, admin bypasses the Pro+ gate), clicked "VandisVIP" in the vendor table → card opened correctly layered on top of the distribution modal, showing a genuine "Payment methods" row with real pills (USDT (Tron), Alibaba, Zelle, Credit card) matching that vendor's actual `pc_vendor_payment_methods` rows. Re-checked the admin Vendors tab afterward to confirm the `PAYMENT_METHODS` extraction didn't break the 20-checkbox payment-method grid on the vendor edit form — renders identically. Zero console errors on either page. `bash deploy.sh` smoke-checked clean.

## [2026-08-02] feature | VendorCard: "Best prices" link — show only products where a vendor has the lowest price

User asked for a way, from the vendor card, to see only the products where that vendor is the cheapest option. The naive approach (filter Comparison to just this vendor's own column, like the existing Catalog link) doesn't actually work for this: `is_lowest` is computed by comparing $/unit across whatever vendors the SQL query returns, so narrowing the query to just one vendor's own rows makes every row trivially "lowest" (nothing left to compare against). Had to keep the full multi-vendor query intact and instead post-filter the computed rows down to only those where the target vendor's entry has `is_lowest = true`.

Backend (`comparison_query.php`): new `cheapest_vendor_id` GET param / `?int $cheapestVendorId` param on `runComparisonQuery()`, applied as a post-filter right after the existing `is_lowest` computation loop (kept entirely separate from the `vendorIds`/`vendors[]` WHERE-clause filter, which must NOT also be active for this to mean anything). Wired through every caller (`comparison/index.php`'s cache-hash/log compact() calls, both export endpoints, `admin/query_log_rerun.php`). Also fixed a latent bug spotted while touching this code: the existing `foreach ($row['vendors'] as &$v) { $v['is_lowest'] = ...; }` loop never `unset($v)` afterward, leaving a dangling by-reference alias that could silently corrupt data if a later loop reused the same `$v` variable name — added `unset($v)` and used a distinctly-named loop variable in the new filter check to be safe regardless.

Frontend: `VendorCard.vue` gained a "Best prices → Where they're cheapest" link (`/comparison?cheapest_vendor_id={id}`, alongside the existing Catalog link). `ComparisonView.vue` gained a dismissible banner ("Showing only products where {vendor} has the lowest price — Clear") and made this filter explicitly mutually exclusive with the vendor-name-search box in both directions: landing via this link clears any active name-search/selectedVendors state (unconditional reset in `initFromQuery()`, unlike the conditional `if (length)` pattern the classification/vendor-id deep links use — needed so re-navigating between the two link types via the same-route query watcher actually clears the old filter instead of leaving it stuck), and typing into the name-search box afterward clears `cheapestVendorId` right back (since narrowing to one vendor's column would make "cheapest" trivially true again).

Caught and fixed a real bug of my own while wiring the mutual-exclusivity logic: the existing `skipVendorSearchWatch` guard flag (from the earlier Catalog-link work) set itself `true` unconditionally before assigning `vendorNameSearch.value` — but Vue's `watch` never fires on a no-op assignment (setting a ref to its current value), so if the target value happened to equal what was already in the box, the guard flag would stay stuck `true` and silently swallow the *next* real keystroke. Fixed by extracting a `setVendorNameSearchSilently()` helper that only arms the guard when the value is actually changing, and used it in both the pre-existing Catalog-link code path and the new Best-prices one.

Verified against real data before trusting the UI: minted a throwaway session, ran the full unfiltered `/api/comparison` query and an independent Python script counting every row where vendor 34 (Anna Yiwu) had `is_lowest = true`, then compared against the actual `cheapest_vendor_id=34` filtered response. Hit an apparent 93-vs-92 mismatch on the first pass — investigated rather than assuming either a false pass or a real bug, and found the real cause: "Glutathione 1200mg" has two separate SKU listings from the same vendor tied at the identical lowest price, so counting raw `is_lowest=true` *entries* (93) versus counting distinct qualifying *rows* (92) are different units — recomputed the qualifying row-set independently and confirmed it matched the filtered response's row-set exactly (92 = 92, identical sets). Live-checked the UI: banner renders correctly, BPC-157 20mg expands to show Anna Yiwu genuinely highlighted as lowest ($75.00/$0.38 per unit) among all 16 real vendors, Clear button restores the full list, and typing a different vendor's name into the search box correctly cancels the banner and switches modes. Zero console errors. `bash deploy.sh` smoke-checked clean, `php -l` clean on all 5 touched backend files.

## [2026-08-02] feature | Cart: add a product without picking a size ("any size")

User asked for a way to add just "BPC-157" to cart without committing to one exact size ("BPC-157 20mg"). Clarified two real design decisions via AskUserQuestion before building: (1) pricing basis for a no-spec item — cheapest $/unit (not raw kit price, which the user explicitly corrected mid-build: comparing kit prices across different doses isn't a fair "cheapest" signal, a 1g bulk listing will always look "cheaper" in kit-price terms than a 5mg vial regardless of actual value); (2) entry point — one "+ Cart (any size)" button per product on the Comparison page's collapsed List-view product row, alongside (not replacing) each individual size's existing button, so both can coexist as separate cart lines for the same product.

**Schema** (migration 042): `pc_cart_items.specification_id` becomes nullable — NULL means "any size of this product." Left the existing 3-column UNIQUE key as-is rather than restructuring it: MariaDB treats every NULL as distinct within a unique index, so it can't catch a duplicate NULL-spec row for the same user+product on its own — `POST /api/cart` (`backend/api/cart/index.php`) checks for an existing NULL-spec row itself before inserting when `specification_id` is omitted.

**Backend** (`backend/lib/cart.php`, `getCartSnapshot()` — substantially rewritten): cart items split into "exact spec" and "any size" groups. For "any size" items: coverage means the vendor carries ANY spec of that product; the price that counts toward a vendor's cart total is THAT VENDOR'S OWN cheapest-$/unit listing among their specs of the product (not their cheapest kit price, and not the market's cheapest — each vendor's own best option, so "if I bought everything from this one vendor" stays a meaningful comparison); the global "cheapest per item" mix-and-match panel picks whichever (vendor, spec) pair has the lowest $/unit market-wide. The winning listing's real `spec_label` is surfaced back to the frontend as `resolved_spec` (e.g. "picked 1g") since the buyer needs to know what size they'd actually receive — the cart item itself doesn't name one.

**Found and fixed a real gap this feature exposed** in `products/merge.php`: the existing `repointCartAndStackItems()` helper (written for a prior "merge silently wipes carts" incident) only repoints cart rows by exact `specification_id` — a NULL-spec "any size" row isn't tied to any spec id, so it would silently fall through and get cascade-deleted when the loser product is removed at the end of a merge, the identical failure class that helper exists to prevent. Added explicit repoint-then-dedup logic for NULL-spec rows right before the loser's delete.

**Two real bugs caught and fixed during verification** (would have shipped broken without testing against live data): (1) `array_unique()` on the spec-id lookup preserved the original (non-sequential) array keys, and PDO's positional `?` parameter binding requires a plain 0-indexed array — threw "Invalid parameter number" on every "any size" cart request; fixed with `array_values()`. (2) The `products/merge.php` fix's first draft used a single self-referencing `DELETE ci FROM pc_cart_items ci WHERE ... EXISTS (SELECT ... FROM pc_cart_items w ...)` — MySQL/MariaDB rejects this exact shape ("Table 'ci' is specified twice"); rewrote as two plain statements (a SELECT to find affected user ids, then a separate DELETE by id list).

**Verified against real data throughout, not just trusting the code**: hand-queried BPC-157's real per-spec cheapest-$/unit rows first to establish ground truth (1g bulk listing at $0.171/mg is the true cheapest, from vendor "Guangzhou Guangjin Trading Co., Ltd." at $171.12) — confirmed the API's `cheapest_by_item` response matched exactly. Spot-checked three specific vendors' own per-vendor "any size" price picks (VandisVIP, Premipeptides, Purelypep Factory) against their real listed rows and confirmed each one picked their OWN lowest-$/unit spec, not their lowest kit price (e.g. VandisVIP's $43 10mg listing at $0.43/mg, correctly beating their cheaper-looking $29 5mg listing at $0.58/mg). Built a full throwaway product-merge scenario (two fake products, a NULL-spec cart row on the loser) end-to-end through the real admin API to confirm the merge fix actually works, including the dedup path (ran it again with a pre-existing NULL-spec row already on the winner, confirmed the loser's duplicate was cleanly dropped rather than crashing). Live-checked the real UI in the browser (own account): added BPC-157 "any size" from Comparison, confirmed it appeared correctly in the Cart page's item list, the per-vendor breakdown, and the "cheapest per item" panel (showing "picked 1g" and the exact same vendor/price as the ground-truth check) — then removed it afterward to leave the cart exactly as found. All throwaway test users/products/sessions deleted. `bash deploy.sh --sync-schema` then `bash deploy.sh` both smoke-checked clean; `php -l` clean on all touched backend files.

## [2026-08-02] fix | Cart "any size" was picking raw/bulk-powder listings over regular finished-vial doses

Same-day follow-up: user reported that "any size" BPC-157 was resolving to the 1g raw/bulk-powder listing ($171.12, since bulk powder's $/mg rate is structurally always going to look "cheapest") instead of a normal finished-vial dose — asked that "any size" be constrained away from raw/tablet specs unless the buyer explicitly picked that exact spec via the per-spec button.

Fixed in `getCartSnapshot()`: joined `pc_specifications` into the price-fetch query (needed `is_raw_material`/`is_tablet` per row, wasn't previously selected) and added `&& !$r['is_raw_material'] && !$r['is_tablet']` to the condition that folds a row into the "any size" accumulation (`$anyBestPerVendor`) — scoped only to that branch, so a deliberately-picked exact raw/tablet spec (added via the normal per-spec "+Cart" button) is completely unaffected. Confirmed a handful of products exist that are *entirely* raw-material with no regular-vial spec at all (e.g. "GHK CU RAW") — for those, "any size" now correctly shows no coverage rather than falling back to the raw listing, matching the user's own framing ("unless that is what is picked" — they'd need to add that exact spec deliberately).

Re-verified against the same BPC-157 ground truth as the original feature: with 1g excluded, the new correct cheapest-$/unit winner is the 20mg spec at vendor Anna Yiwu, $75.00 — confirmed the live API now returns exactly that (previously returned the $171.12 1g listing). `bash deploy.sh` smoke-checked clean, `php -l` clean.

**Incident during verification**: while re-testing this fix live in the browser, several clicks meant for the search box instead landed on "+ Cart (any size)" buttons on other product rows (a UI focus/coordinate issue, not a logic bug), adding 10 unintended items to the user's real cart. Caught it via a follow-up screenshot, identified the exact accidental rows by their `specification_id IS NULL` signature via direct SQL, and removed only those — the two real items already in the cart (GHK-Cu 100mg, Retatrutide 20mg) were left untouched and confirmed correct afterward. No lasting impact, but noted here since it's a real "mutated the user's live data during verification" event, not just a throwaway-test cleanup.

## [2026-08-03] fix | Slow-query audit: missing pc_price_history index + Calendar milestones N+1 rewrite

User asked to pull the slow-query log and see if any indexes or caching needed attention. Queried `pc_slow_query_cache` directly (1,933 "new" entries) and clustered by real query shape rather than treating each row as independent — most of the volume traced to one root cause: `pc_price_history`'s only index is `(vendor_id, product_id, specification_id, changed_at)` (leading column `vendor_id`), so any query needing "history for this product+spec across ALL vendors" — which the Calendar feature does constantly — can't use it at all and falls back to a near-full-table scan (5,000-6,400 rows examined for a 2-3 column filter, out of ~8,700 total rows).

Root cause was concentrated in `getCalendarMilestones()` (`backend/lib/calendar_featured.php`, backlog #19's all-time-low detector): (1) its outer "which pairs changed this month" query wrapped `changed_at` in `DATE_FORMAT(..., '%Y-%m') = ?`, which defeats the separate `changed_at` index outright (function-wrapped indexed columns aren't sargable); (2) it then looped per distinct (product, spec) pair and re-queried the *entire* history table once per pair (genuine N+1 — real production data showed 46-75 occurrences of dozens of distinct per-pair queries, each a near-full-table scan). It's cached 10 minutes (`calendar_data` group) but that cache gets busted very frequently in normal operation — Review Queue approvals (which happen in large batches per this project's history), vendor file imports, and manual price edits all bust it — so in practice this expensive computation re-ran often, not just once per 10-minute window.

Presented findings to the user before touching anything (per this project's own "stage consequential fixes" convention) — confirmed the admin Vendors/Products list queries and the main Comparison/ranked.php aggregate queries were lower priority (already cached and reasonably indexed; their volume reflects normal admin-panel cache churn, not a bug) and got explicit go-ahead on the two real fixes.

**Fix 1** — migration 043: added `pc_price_history.idx_product_spec_tier (product_id, specification_id, tier_kit_size, changed_at)`, pure additive, benefits every history-across-vendors lookup automatically (also speeds up `getCalendarFeatured()`'s per-featured-day delta lookup and `products/merge.php`'s history-repointing UPDATE, with zero code changes needed for those).

**Fix 2** — rewrote `getCalendarMilestones()`: replaced the `DATE_FORMAT()` filter with a sargable `changed_at >= ? AND changed_at < ?` range, and replaced the per-pair N+1 loop with one batched query (`WHERE (product_id, specification_id) IN (...)` row-constructor, same pattern already used in `cart.php`) ordered so each pair's rows land contiguously, letting a single pass track a running per-pair minimum instead of a query per pair.
...
## [2026-08-03] feature | Vendor Inventory tab: add a brand-new product/price line directly

User asked for a way to add a new product to a vendor from the admin Inventory tab (`InventoryTab.vue` — full per-vendor price-line editing, previously edit-only: no way to add a line that didn't already exist, only the file-upload/Claude-extraction pipeline or Review Queue could create new price rows).

Backend: new `POST /vendors/{id}/prices` (`backend/api/vendors/add_price.php`), reusing the exact same product/spec resolution helpers the Review Queue approval flow already uses (`findExactProductMatch()`, `createProduct()`, `findOrCreateSpec()`, `commitPriceRow()` — all from `price_import.php`), so this new manual path can't drift from the established commit logic. Product resolution offers the same two explicit choices as the Review Queue's own picker: an existing `product_id`, or `create_new: true` + a name — never a silent guess. Extended `commitPriceRow()` with optional `$historySource`/`$changedBy` params (defaulting to the existing `'import'`/null, so the two pre-existing callers are unaffected) so this new manual-add path correctly logs `source='manual_edit'` + the admin's own id in `pc_price_history`, instead of misleadingly reusing `'import'` for something that was never a file import.

Frontend: `InventoryTab.vue` gained a "+ Add product" toggle button (next to "Recalculate $/unit", only shown once a vendor is selected) that reveals an inline form — product name + a search/select product picker (same pattern as the Review Queue's, defaulting safely to "create new" rather than pre-selecting a guess), spec label/value/unit, price/tier/vials-per-kit/SKU, and the same raw-material/tablet/non-standard-kit checkboxes already used elsewhere. Submits, reloads the vendor's price list, and collapses the form on success.

Verified against real data via a throwaway test vendor: (1) added a price against an *existing* product (BPC-157) with a brand-new 77mg spec — confirmed the price row, correct `price_per_unit` computation, and a `pc_price_history` row correctly tagged `source='manual_edit', changed_by=<admin id>` (not `'import'`); (2) added a price with `create_new: true` — confirmed a genuinely new `pc_products` row was created and priced correctly; (3) confirmed the validation error path (missing spec label/value/price) returns a clear 422 rather than a bare SQL error. Live-checked the real Inventory tab in the browser: the form renders correctly, and typing into the product-name field correctly live-filters the picker (confirmed via the actual `<select>` options, including the "Create new product" option text updating to match). All throwaway vendor/product/price/history/session rows deleted afterward, confirmed gone. `bash deploy.sh` smoke-checked clean, `php -l` clean on all touched/new backend files.

## [2026-08-05] feature | Inventory tab: relabel a mislabeled dose on one price row (10mg → 15mg fix)

User needed to correct a specific vendor's price line where the vendor had mislabeled the dose (Premipeptides' "Tesamorelin + Ipamorelin combo" listed as 10mg, actually 15mg). No existing capability could do this — `PUT /api/prices/{id}` (Inventory tab's edit form) explicitly could not reassign which spec a row belongs to; only `products/spec_move.php` existed, and that moves a whole spec (every vendor's prices under it) to a different *product*, not one vendor's one row to a different *spec* on the same product. Found the real target row via direct SQL first (vendor 10, product 312, spec 1023/"10mg", price_id 5113) and confirmed a "15mg" spec (id 1375) already existed on that product from other vendors — so the fix needed to *repoint* onto the existing spec, not create a duplicate.

Built the capability rather than just hand-fixing the one row, since the underlying gap (no way to correct a mislabeled spec at all) would recur. `prices/update.php` gained optional `spec_label`/`numeric_value`/`unit`/`is_raw_material`/`is_tablet` fields, resolved via the same `findOrCreateSpec()` every import path already uses — repoints onto an existing sibling spec on the product if the label matches one (the common case, exactly this scenario), or creates a new spec otherwise. Moved the spec resolution to run *before* the existing price_per_unit computation, since a pure relabel (same price/kit) still needs $/unit recomputed against the corrected dose — extended that computation's trigger condition accordingly. Deliberately did NOT log a `pc_price_history` event for a pure relabel with no price/kit change (it's a data-correctness fix — "this was always 15mg" — not a real price-change event that belongs on the Calendar). `vendors/show.php` (which feeds the Inventory tab) now also selects `numeric_value`/`unit`/`is_raw_material`/`is_tablet` per row so the frontend can pre-fill and preserve them.

Frontend: the Spec column in `InventoryTab.vue` changed from plain read-only text to three small inputs (value/unit/label), matching every other directly-editable field in that table — editing the value/label and blurring triggers the same `save()` used everywhere else, which now always includes the spec fields (a no-op re-resolution in the common case; only actually repoints the row if the admin changed them) and patches the row from the server's authoritative resolution afterward rather than trusting its own typed input.
