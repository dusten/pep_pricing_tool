...
**2026-07-23 reinforcement:** don't even run `npm run build` standalone as a pre-check/"verify it compiles" step before deploying — that's still an extra manual step the user has to watch happen for no reason. `deploy.sh` runs the build itself and will surface any build error right there in its own output. Just run `bash deploy.sh` directly as the first and only build/ship action, skip the separate build-then-deploy two-step entirely.
