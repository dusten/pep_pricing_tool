...
- This does **not** apply to purely diagnostic one-liners (e.g. dumping a raw API response to debug a failure) or deliberately-rolled-back test transactions that never persist anything — only scripts with a real, lasting effect on production data belong here.
