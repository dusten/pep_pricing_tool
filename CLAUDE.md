...
4. **ClamAV scan on vendor file uploads — built, but currently disabled and broken in production.** `backend/lib/malware_scan.php` is fully wired into `backend/api/vendors/files.php` (gated behind a `MALWARE_SCAN_ENABLED` flag). As of 2026-07-02: the flag is `false` on prod, and `clamd@scan.service` is crash-looping (exit code 1, systemd gave up restarting it — matches the "clamd@scan restart failed" warning deploy.sh has been printing). Uploaded vendor files are not currently being scanned at all. Needs the daemon's crash root-caused before the flag can be safely re-enabled.
